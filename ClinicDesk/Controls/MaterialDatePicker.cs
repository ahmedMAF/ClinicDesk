...
{/*
    ...
    public MaterialDatePicker()
    {
        ...
        Font font = SkinManager.GetFontByType(FontType.H6);
        TopDayFont = new Font(font.Name, 10, FontStyle.Regular);
        MonthFont = new Font(TopDayFont.Name, 16, FontStyle.Bold);
        DayFont = new Font(TopDayFont.Name, 40, FontStyle.Bold);
        YeahrFont = new Font(TopDayFont.Name, 15, FontStyle.Bold);
        DoubleBuffered = true;
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        DrawShadow(g, ShadowPath, 8, SkinManager.GetCardsColor());
    ...
    private static void DrawShadow(Graphics g, GraphicsPath gp, int d, Color pBackColor)
    {
        Color[] colors = [.. getColorVector(Color.Black, pBackColor, d)];

        for (int i = 0; i < d; i++)
        {
            g.TranslateTransform(1f, 0.75f);        // <== shadow vector!
            using Pen pen = new(colors[i], 1.75f);  // <== pen width (*)
            g.DrawPath(pen, gp);
        }
